...
int main()
{
    ...
    while (ch != 27 && matou_todos != QtdNinjas && naruto.vidas>0)
    {
        ...
        if (kbhit())
        {
            ...
            switch (toupper(ch))
            {
                case 32:
                    ...
                    {
                        atualiza_xys = 0;
                        atira(&pos_shuriken, ch, prox_ch, mapa, NINJA, ninja_morto, &matou_todos, &i, &atualiza_xys, QtdNinjas);
                        flag_tiro = 1;
                        naruto.shurikens--;
                    }
                    break;
        ...
        if (tempo.duracao >= TIME_GAME && flag_ninja != 1)
        {
            ...
            {
                if (naruto.y == NINJA[n].y)
                    atira_ninja(ch, prox_ch, mapa, &naruto, NINJA, ninja_morto, &tempo, &flag_ninja, &matou_todos, &pos_arm, &chaves, QtdNinjas);
            }
            flag_ninja = 1;
        }
        for (l = 0; l < LIN; l++)
